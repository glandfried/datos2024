...
\begin{textblock}{160}(0,4)
 \centering \LARGE La Inteligencia Humana\\ \large
   \only<1->{La emergencia de la cultura}
\end{textblock}
...
Si nos va cada vez mejor en el entrenamiento, \\ nos debería ir mejor en la evaluación
\end{textblock}
}
...
\only<7>{
\begin{textblock}{105}(53,28) \centering \Large
\begin{flalign*}
& \text{Riqueza inicial: } 100 \\[0.3cm]
& \text{Riqueza actual: } \\[-0.7cm] \large
& \text{\ \ \ $\circ$ Si sale Cara: } \overbrace{(100 \cdot 0.5)}^{50} \cdot \overbrace{Q_c}^{3} = 150 \text{ \ \ \ 50\%} \\
& \text{\ \ \ $\circ$ Si sale Sello: } (100 \cdot 0.5) \cdot \underbrace{Q_s}_{1.2} = 60 \text{ \ \ \ \,-40\%}
\end{flalign*}
\end{textblock}
}
...
más que las ganancias \\[0.1cm] \large
...
\only<14>{
\begin{textblock}{140}(10,38) \centering \Large
Cooperar es óptimo incluso \\ desde un punto individualista
\end{textblock}
}
...
\end{frame}
 ...
 \begin{frame}[plain]
\begin{textblock}{160}(0,04) \centering \LARGE
Historia reciente \\[0.4cm] \large

\only<1>{Mientras Europa vivía la ``Edad Oscura'' (700-1500) \\
el resto del mundo florecía de sociedades prósperas}%
\only<2>{En 1546 los migrantes feudales descubren la plata americana, \\
adoptada como moneda oficial en China 200 años antes.}%
\only<3>{Durante 300 años Europa usa la plata americana para importar productos.\\
Inglaterra hace su primera revolución industrial a base a tecnología China.}%
\only<4>{En 1840 los ingleses financian su déficit comercial con China mediante el narcotráfico\\
China le hace la guerra al narcotráfico en su propio territorio y la pierde en 1842 y 1860.}%
\only<5>{Comienza la era de los genocidios y pérdida masiva de diversidad cultural. \\
Europa invade los territorios americanos libres y entra por primera vez a África.
}
\only<6>{Como consecuencia de la pérdida de diversidad cultural \\
vivimos en la actualidad una masiva pérdida de biodiversidad.}
\only<7>{A pesar de todos los avances, la ciencia no es capaz de compensar la pérdida \\
de conocimientos milenarios y la crisis ecológica actual que no deja de profundizarse.
}
\end{textblock}
...
\includegraphics[width=0.6\textwidth]{static/mapaMundial.jpg}
\end{textblock}
}

\only<2>{
\begin{textblock}{160}(0,24) \centering
\includegraphics[width=0.6\textwidth]{static/plata-potosi.jpg}
\end{textblock}
}

\only<3>{
\begin{textblock}{160}(0,24) \centering
\includegraphics[width=0.36\textwidth]{static/scienceAndCivilizationChina2.jpg}
\end{textblock}
}

\only<4>{
\begin{textblock}{160}(0,24) \centering
\includegraphics[width=0.66\textwidth]{figuras/china.pdf}
\end{textblock}
}

\only<5>{
\begin{textblock}{160}(0,24) \centering
\includegraphics[width=0.44\textwidth]{static/genocidio_patagonia.jpg}

\includegraphics[width=0.44\textwidth]{static/africa.png}
\end{textblock}
\begin{textblock}{160}(0,25) \centering
 \small Patagonia, Argentina
\end{textblock}
}

\only<6>{
\begin{textblock}{70}(10,24) \centering
\includegraphics[width=0.975\textwidth]{static/chaco1984.jpg}
\end{textblock}
\begin{textblock}{70}(80,24) \centering
\includegraphics[width=0.975\textwidth]{static/chaco2022.jpg}
\end{textblock}
}

\only<7>{
\begin{textblock}{160}(0,24)  \centering
\includegraphics[width=0.7\textwidth]{static/chatGPT.jpeg}
\end{textblock}
}
...
\begin{frame}[plain]
\begin{textblock}{160}(0,4)  \centering \LARGE
Inteligencia Causal \\ \large
\only<7->{¿Qué (inter)acciones generan nuestro bienestar?}\phantom{.}\\[1.3cm]
\only<8>{\Large Principios del buen vivir, versión Aymara.}
\end{textblock}

\begin{textblock}{150}(05,18)
\only<2-6>{\ \ $\circ$ Evalúa todo el espacio de hipótesis mediante el principio de ``no mentir'': máxima incertidumbre dada la información disponible. \\[0.3cm]}

\only<3-6>{\ \ $\circ$ Haciendo uso de nuestro conocimiento sobre la realidad causal, obtenemos desempeños que no pueden ser superados por ningún modelo de Inteligencia Artificial. \\[0.3cm]}

\only<4-6>{\ \ $\circ$ La Inteligencia Causal nos lleva a preferir modelos simples que, en un contexto de crisis ecológica, gastan menos energía que los modelos de Inteligencia Artificial. \\[0.3cm]}

\only<5-6>{\ \ $\circ$ A diferencia de la Inteligencia Artificial ``de punta'', solo desarrollada por grandes empresas, la Inteligencia Causal puede ser desarrollada por cualquier profesional.\\[0.3cm]}

\only<6>{\ \ $\circ$ Los modelos causales permiten explicar el mundo y tomar decisiones, lo que no ocurre con los modelos Inteligencia Artificial, que son de caja negra (no interpretables).}
\end{textblock}

\begin{textblock}{155}(2.5,18)
\only<9->{$1.$ \textit{Suma Umaña}: Saber beber. El agua es la fuente de toda de vida.}

\only<10->{$2.$ \textit{Suma Manq'aña}: Saber comer. Nuestro alimento es la vida de otros seres.}

\only<11->{$3.$ \textit{Suma Thokoña}: Saber bailar. El cuerpo es la conexión con el cosmos.}

\only<12->{$4.$ \textit{Suma Ikiña}: Saber dormir. Para recuperar la energía de dos días.}

\only<13->{$5.$ \textit{Suma Irnakaña}: Saber trabajar. Nuestra acción cumple un rol en la comunidad.}

\only<14->{$6.$ \textit{Suma Lupiña}: Saber meditar. El silencio restablece los equilibrios.}

\only<15->{$7.$ \textit{Suma Amuyaña}: Saber pensar. Sin perder la razón, caminamos la senda del corazón.}

\only<16->{$8.$ \textit{Suma Munaña Munayasiña}: Saber amar y ser amado. Coexistir por complementariedad.}

\only<17->{$9.$ \textit{Suma Ist'aña}: Saber escuchar. Percibir incluso aquello que no habla ni se muestra.}

\only<18->{$10.$ \textit{Suma Aruskipaña}: Saber hablar. Nuestras acciones se escriben en otros corazones.}

\only<19->{$11.$ \textit{Suma Samkasiña}: Saber soñar. Soñar es la semilla de la nueva realidad.}

\only<20->{$12.$ \textit{Suma Sarnaqaña}: Saber caminar. No existe el cansancio para quien sabe caminar.}

\only<21->{$13.$ \textit{Suma Churaña, suma Katukaña}: Saber dar y recibir con dicha y agradecimiento, porque la vida es la unión de muchos seres y fuerzas.}
\end{textblock}
...
\includegraphics[width=1\textwidth]{static/egipto5.jpeg}
\end{textblock}
}
\only<2>{
\begin{textblock}{160}(0,0)  \centering
\includegraphics[width=1\textwidth]{static/egipto4.jpeg}
\end{textblock}
}
\only<3>{
\begin{textblock}{160}(0,0)  \centering
\includegraphics[width=1\textwidth]{static/egipto3.jpeg}
\end{textblock}
}
\only<4>{
\begin{textblock}{160}(0,0)  \centering
\includegraphics[width=1.25\textwidth]{static/egipto2.jpeg}
\end{textblock}
}
\only<5>{
\begin{textblock}{160}(0,0)  \centering
\includegraphics[width=1.25\textwidth]{static/egipto1.jpeg}
\end{textblock}
}


\begin{textblock}{160}(02,4) \LARGE
\textcolor{white}{El conocimiento emerge como la vida}
\end{textblock}
